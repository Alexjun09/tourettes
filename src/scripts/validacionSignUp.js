...
document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", async function (event) {
      ...
      const nombreCompletoValid = validateNombreCompleto(
      ...
      const nombreUsuarioValid = await checkUsernameAvailability(
        form.nombreUsuario.value
      );
  ...
  function validateContrasena(contrasena) {
    if (
      !/[A-Za-z].*[0-9]|[0-9].*[A-Za-z]/.test(contrasena) ||
      contrasena.length < 8 ||
      !/[A-Z]/.test(contrasena)
    ) {
      showError(
        "contrasena",
        "La contraseña debe tener al menos 8 caracteres alfanuméricos y una mayúscula"
      );
      return false;
    }
    return true;
  ...
  async function checkUsernameAvailability(nombreUsuario) {
    // Comprueba si el nombre de usuario está vacío o solo contiene espacios.
    if (!nombreUsuario.trim()) {
      showError("nombreUsuario", "El nombre de usuario no puede estar vacío");
      return false;
    }
